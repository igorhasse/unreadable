...
- `.github/workflows/deploy.yml` runs `yarn install --immutable && yarn vinext deploy` on push to `main` with `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID` secrets. **Don't use `yarn dlx vinext deploy`** — dlx runs in an isolated `/tmp/.../dlx-...` folder that doesn't see project peer deps (vite, react, react-dom, @vitejs/plugin-react), so vinext crashes with `ERR_MODULE_NOT_FOUND: Cannot find package 'vite'`. The local-bin invocation (`yarn vinext`) resolves peers from the project tree.

## Key vinext / Cloudflare Workers gotchas

- **`next/font/google` self-hosts at build time, but silently falls back to a render-blocking external `<link>` if the build-time fetch fails.** vinext caches the woff2 + CSS into `/_vinext_fonts/...` and rewrites `@font-face` URLs to local paths. But `vinext/dist/plugins/fonts.js` has a `} catch { return; }` around the cache step — any fetch flake (rate limit, transient network) means that family ships as `<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=...">` in the critical path, with **no warning, no error**. We hit this with `JetBrains_Mono` (Newsreader and Geist worked fine). Workaround when a family persistently falls back: drop the `next/font/google` import for it and self-host manually — put the woff2 in `public/fonts/`, declare `@font-face` **inline in `CRITICAL_CSS`** in `app/[locale]/layout.tsx` (not in `tokens.css` — declaring `@font-face` there inflates the main CSS chunk), and add `fonts/.+` to the `proxy.ts` matcher exclusion so the woff2 isn't locale-redirected.
- **No `font-size-adjust` fallback metrics** from `next/font/google` (per vinext README) — same family fallback chain you set in `--font-*` is what users see during swap.
...
- **`vinext start` does NOT compress assets locally.** Cloudflare's CDN serves brotli/gzip in production, so a Lighthouse run against `http://localhost:3000` overstates transfer sizes by 4–5× for text assets. CSS/JS-heavy regressions seen locally usually evaporate in prod. When perf-tuning, also gzip the artifact (`gzip -9 -c dist/client/assets/*.css | wc -c`) or just measure on `igorhasse.com` after deploy.
- **Vite/vinext caches stick around between builds.** When build output looks stale (e.g., a removed font import still appearing in `dist/server/index.js`), clear `node_modules/.vite` + `dist/` + `.vinext/` and rebuild. Hash-named chunks like `index-lB-XYn8Q.css` are content-addressed, so the same hash reappearing across builds is normal — what matters is the file's content.
