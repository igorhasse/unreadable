...
**The Shiki bug.** Tried to use shiki with the default engine (oniguruma WASM). Crashed in production with Cloudflare error 1101. `WebAssembly.instantiate()` from bytes is blocked in Workers. AI proposed `createJavaScriptRegexEngine`. Worked first try. But it didn't have that knowledge alone. I dug into the Cloudflare docs, found the restriction, came back to it and asked it to adapt. Total iteration: 15 minutes. Without me, it would've kept circling the 1101 error without ever reaching the architectural restriction underneath.

**The skills pipeline.** At some point I decided I wanted to write in my own voice without AI writing for me. We built two skills: `personal-voice` (converts a raw draft into my voice) and `editorial-template` (structures polished prose into a publishable bundle). The first has `voice-profile.md` with verbatim citations from my corpus. The second has `dan-patterns.md` based on overreacted.io. Each skill, ONE responsibility. AI uses these skills every time I write now, and the result stays auditable. I compare the raw draft with the output and see exactly what changed.
